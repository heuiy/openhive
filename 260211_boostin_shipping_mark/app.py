...
def api_generate():
    ...
    try:
        ...
        for b in range(batch_count):
            pdf_file = request.files.get(f"pdf_{b}")
            if pdf_file is None:
                return jsonify({"error": f"배치 {b+1}의 PDF 파일이 없습니다."}), 400
...
def api_preview():
    ...
    try:
        ...
        pdf_file = request.files.get("pdf_0")
        if pdf_file is None:
            return jsonify({"error": "미리보기를 위해 첫 번째 배치의 PDF를 업로드하세요."}), 400
